...
def normalizar_jid(jid, push_name=None):
    ...
    1️⃣ pushName
    2️⃣ profilePicUrl parcial dinámico
    ...
    if push_name and push_name.strip():
     for c in contactos:
        name = str(c.get("pushName", "")).lower().strip()
        if push_name.lower().strip() == name and c["remoteJid"].endswith("@s.whatsapp.net"):
            print(f"🎯 Coincidencia EXACTA por nombre: {push_name} → {c['remoteJid']}")
            return c["remoteJid"]

    # 2️⃣ Coincidencia por foto (solo si pushName existe)
    if push_name and push_name.strip():
        for c in contactos:
            url = c.get("profilePicUrl", "")
            if url and "whatsapp.net" in url and c["remoteJid"].endswith("@s.whatsapp.net"):
                # Coincidencia dinámica: intenta buscar pushName dentro de la URL (ejemplo)
                if push_name.lower().replace(" ", "") in url.lower().replace(" ", ""):
    ...
    # ❌ Si no hay coincidencia confiable, no inventar
...
def imprimirresultado():
    webhook = {
        "key": {"remoteJid": "268113450905d721@lid"},
        "pushName": "< Davida />",
